...
from spark_tasks.data.mug import MUG_CFG


@configclass
class ObjectTableSceneCfg(object_table_env_cfg.ObjectTableSceneCfg):
    mug: RigidObjectCfg = MUG_CFG.replace(
        prim_path="/World/envs/env_.*/mug",
    )
    mug.init_state = RigidObjectCfg.InitialStateCfg(
        pos=(0.45, 0.0, 0.758),
        rot=(0.6065, -0.3590, 0.5960, -0.3849),
    )
    mug.spawn.scale = (1.7, 1.7, 1) # 不是很能确认对于g1来说杯子的scale是否合适 可能需要更改
...
class EventCfg:
    ...
    reset_flip_mug = EventTerm(
        func=mdp.reset_flip_mug_objects,
        mode="reset",
        params={
            "mug_cfg": SceneEntityCfg("mug"),
            "randomize": False,
