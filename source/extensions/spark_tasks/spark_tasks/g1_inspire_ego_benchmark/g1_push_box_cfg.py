...
from spark_tasks.data.marker import MARKER_CFG
...
class EventCfg:
    ...
    reset_push_box = EventTerm(
        func=mdp.reset_push_box_objects,
        mode="reset",
        params={
            "box_cfg": SceneEntityCfg("box"),
            "default_goal_pos": (0.5, 0.0, 0.687),  # Same as G1InspireFTPEnvCfg.goal_default_pos
        },
    )
...
class G1InspireFTPEnvCfg(ManagerBasedRLEnvCfg):
    ...
    # Goal marker config (not part of scene, created at runtime)
    goal_cfg: VisualizationMarkersCfg = MARKER_CFG.replace(prim_path="/Visuals/goal")
    goal_default_pos = (0.5, 0.0, 0.687)  # Adjusted for Spark table height
