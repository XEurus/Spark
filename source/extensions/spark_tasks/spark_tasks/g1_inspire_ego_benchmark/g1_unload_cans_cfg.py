...
'''
2025-12-02T09:40:59Z [57,758ms] [Error] [omni.physx.plugin] PhysX error: PxRigidDynamic::setLinearVelocity: Body must be non-kinematic!, FILE /builds/omniverse/physics/physx/source/physx/src/NpRigidDynamic.cpp, LINE 232
2025-12-02T09:40:59Z [57,758ms] [Error] [omni.physx.plugin] PhysX error: PxRigidDynamic::setAngularVelocity: Body must be non-kinematic!, FILE /builds/omniverse/physics/physx/source/physx/src/NpRigidDynamic.cpp, LINE 266
2025-12-02T09:40:59Z [57,759ms] [Error] [omni.physx.plugin] PhysX error: PxRigidDynamic::setLinearVelocity: Body must be non-kinematic!, FILE /builds/omniverse/physics/physx/source/physx/src/NpRigidDynamic.cpp, LINE 232
2025-12-02T09:40:59Z [57,759ms] [Error] [omni.physx.plugin] PhysX error: PxRigidDynamic::setAngularVelocity: Body must be non-kinematic!, FILE /builds/omniverse/physics/physx/source/physx/src/NpRigidDynamic.cpp, LINE 266
模型可以正常加载，但是不知道控制层面是否存在一些问题。暂时无法确定问题来源和表现，也许需要进行一些测试。
'''
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        """
        这里位置获取的是否正确暂时存疑 原有版本是针对当前环境原点的坐标，减去当前环境原点。
        单环境内理论上似乎不影响。不知道manage-base会不会自动处理。需要进行测试？
        task_done_unload_cans中使用的不是这种读取方式，做了手动处理。
        以及比原有多获取了一个container 
        """
...
class TerminationsCfg:
    ...
    success = DoneTerm(
        func=mdp.task_done_unload_cans,
        params={
...
class EventCfg:
    ...
    reset_unload_cans = EventTerm(
        func=mdp.reset_unload_cans_objects,
        mode="reset",
        params={
            #"randomize": True,
            #"randomize_idx": -1,
            #"randomize_range": 1.0, #不进行随机位置初始化
            "can1_cfg": SceneEntityCfg("can_fanta_1"),
            "can2_cfg": SceneEntityCfg("can_fanta_2"),
